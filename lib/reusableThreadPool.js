...
class ReusableThread extends EventEmitter {
  ...
  _startWorker() {
    ...
    const workerUrl = pathToFileURL(this.workerFile);
    this.worker = new Worker(workerUrl, { workerData: { imports: this.imports } });
...
class ThreadPoolReusable extends EventEmitter {
  constructor(size = 4, { imports = [] } = {}) {
    ...
    for (let i = 0; i < this.size; i++) {
      const t = new ReusableThread(WORKER_FILE, { imports: this.imports });
